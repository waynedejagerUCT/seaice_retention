...
# Load the file
ds = xr.open_dataset(f"/home/waynedj/Projects/seaiceretention/trajectories/distances/distance_data_Parcel_NEMO_2deg_06hr_v{version}.nc")

# data variables at time t
t                      = 89
cum_distances          = ds["cum_distance"].isel(time=t).values
ref_displacement       = ds["ref_displacement"].isel(time=t).values
meandering_coefficient = ds["meandering_coefficient"].isel(time=t).values

# Reference lon/lat at time t0
t0            = 0
ref_longitude = ds["longitude"].isel(time=t0).values
ref_latitude  = ds["latitude"].isel(time=t0).values
...
# Map settings shared
extent     = [-3500000, 3500000, -3500000, 3500000]  # adjust region
vmin, vmax = 0, 1600  # km
...
pcm1 = axes[0].pcolormesh(lon_2D, lat_2D, cum_distances.reshape(lon_2D.shape),transform=ccrs.PlateCarree(), cmap=CustomColorMap1(), vmin=vmin, vmax=vmax)
...
pcm2 = axes[1].pcolormesh(lon_2D, lat_2D, ref_displacement.reshape(lon_2D.shape),transform=ccrs.PlateCarree(), cmap=CustomColorMap1(), vmin=vmin, vmax=vmax)
...
pcm3 = axes[2].pcolormesh(lon_2D, lat_2D, meandering_coefficient.reshape(lon_2D.shape),transform=ccrs.PlateCarree(), cmap=CustomColorMap2(), vmin=1, vmax=8)
...
plt.savefig(f"/home/waynedj/Projects/seaiceretention/trajectories/distances/distance_data_Parcel_NEMO_2deg_06hr_v{version}/fig_{version}_t{t+1:03d}days.png", dpi=500, bbox_inches='tight')
# %%
